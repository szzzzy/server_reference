...
def set_hardware_sleep(ser, background_dbfs, read_frame_fn=None):
    # Arm only after the board has really become quiet. A fixed delay is not
    # reliable because different prompts leave different speaker/I2S tails.
    quiet_needed = 30  # 30 x 20 ms = 600 ms continuous quiet
    quiet_frames = 0
    quiet_deadline = time.monotonic() + 4.0
    while read_frame_fn is not None and quiet_frames < quiet_needed and time.monotonic() < quiet_deadline:
        try:
            _, frame_dbfs, _ = read_frame_fn(ser)
            if frame_dbfs <= background_dbfs + 3.0:
                quiet_frames += 1
            else:
                quiet_frames = 0
        except TimeoutError:
            break
    # Remove any continuous PCM already queued before switching modes, so it
    # cannot be mistaken for a fresh hardware sound trigger.
    ser.reset_input_buffer()
    value = int(round(background_dbfs * 100)) & 0xFFFFFFFF
    rp.serial_write_all(ser, b"MICS" + value.to_bytes(4, "little"))
    ser.flush()
    time.sleep(0.08)
    ser.reset_input_buffer()


def set_continuous_microphone(ser):
    rp.serial_write_all(ser, b"MICW" + (0).to_bytes(4, "little"))
    ser.flush()


def main():
    print("VERSION=20260823-two-stage-hardware-wake-v5", flush=True)
    ...
    from board_serial_asr_test import (
        capture_seconds, capture_until_endpoint, discard_buffered_audio, read_frame,
    ...
    # Connection is handled by wait_and_connect_board below. Keeping ser=None
    # here makes a temporarily occupied COM port retry instead of crashing V5.
    ser = None
    ...
    def wait_for_hardware_sound_trigger(background_dbfs):
        # MICS stops continuous PCM upload. The board locally checks only the
        # sound level; any sound above the threshold may start a short upload.
        set_hardware_sleep(ser, background_dbfs, read_frame)
        status_file.write_text("hardware_sleep_waiting_for_sound", encoding="utf-8")
        print("硬件休眠：不上传PCM、不保存录音、不运行ASR；开发板仅本地计算声音能量。", flush=True)
        print(f"一级阈值：背景 {background_dbfs:.2f} dBFS + 5 dB，连续120毫秒。", flush=True)
        silent_checks = 0
        missing_port_checks = 0
        while True:
            try:
                first_frame = read_frame(ser)
                print("一级声音阈值已触发，临时识别唤醒词。", flush=True)
                return first_frame
            except TimeoutError:
                # No PCM during hardware sleep is the expected condition.
                if ser.port in {item.device for item in list_ports.comports()}:
                    missing_port_checks = 0
                else:
                    missing_port_checks += 1
                    if missing_port_checks >= 3:
                        raise ConnectionError(f"开发板串口连续三次未检测到：{ser.port}")
                silent_checks += 1
                if silent_checks % 3 == 0:
                    print("[休眠实证] 最近约6秒收到0个PCM帧，5090未录音、未运行ASR。", flush=True)
                continue
    ...
    def run_board_session(background_dbfs, state):
        while True:
            if state == "wake":
                first_frame = wait_for_hardware_sound_trigger(background_dbfs)
                # Do not discard here: the board has just uploaded its 500 ms
                # pre-roll, which preserves the beginning of the wake phrase.
                # The first frame is returned by the trigger wait and is
                # retained as part of the temporary wake-word recording.
                first_seq, first_db, first_samples = first_frame
                try:
                    tail, _, endpoint = api["capture_until_endpoint"](
                        ser,
                        max_seconds=config["voice_control"]["wake_listen_seconds"],
                        background_dbfs=background_dbfs,
                        endpoint_silence_ms=config["voice_control"].get("wake_endpoint_silence_ms", 500),
                        threshold_above_bg=mode["start_threshold_db"],
                        endpoint_threshold_above_bg=mode["end_threshold_db"],
                        endpoint_active_penalty=mode["endpoint_active_penalty"],
                        voice_start_ms=20,
                        voice_start_window_ms=500,
                    )
                except TimeoutError:
                    print("一级触发后未形成完整唤醒语音，正常重新进入硬件休眠。", flush=True)
                    continue
                samples = __import__("numpy").concatenate([first_samples, tail])
                text, _, _ = transcribe(samples)
                value = normalize(text)
                if any(normalize(word) in value for word in config["voice_control"]["wake_words"]):
                    set_continuous_microphone(ser)
                    speak(prompt_texts["menu"], "menu")
                    status_file.write_text("awake_waiting_for_command", encoding="utf-8")
                    state = "command"
                else:
                    print("未识别到唤醒词，重新进入硬件休眠。", flush=True)
                continue
            if state == "command":
            ...
            if not endpoint.get("speech_started"):
                if state == "command":
                    print("未检测到“开启对话”，返回硬件休眠。", flush=True)
                    status_file.write_text("sleeping_waiting_for_wake_word", encoding="utf-8")
                    state = "wake"
                continue
            text, _, _ = transcribe(samples)
