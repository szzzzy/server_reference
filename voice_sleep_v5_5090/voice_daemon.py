...
def set_hardware_sleep(ser, background_dbfs):
...
def main():
    ...
    def wait_for_hardware_sound_trigger(background_dbfs):
        ...
        set_hardware_sleep(ser, background_dbfs)
        status_file.write_text("hardware_sleep_waiting_for_sound", encoding="utf-8")
        print("硬件休眠：不上传PCM，等待任意声音达到一级阈值。", flush=True)
        while True:
            try:
    ...
    def run_board_session(background_dbfs, state):
        while True:
            if state == "wake":
                ...
                tail, endpoint = api["capture_until_endpoint"](
                    ser,
