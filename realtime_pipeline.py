...
def board_speaker_chunk(ser, pcm, max_bytes=4096):
    if len(pcm) % 2:
        pcm = pcm[:-1]
...
def tts_stream_to_board(queue, request_id, text, output_wav, ser, chunk_bytes=4096, timeout=180):
    request = queue / f"request_{request_id}.json"
    response = queue / f"response_{request_id}.json"
    ...
    while time.time() < deadline:
        ...
        if chunk_path.exists():
            ...
            board_speaker_chunk(ser, pcm, chunk_bytes)
...
def main():
    ...
    port = input(f"开发板串口（默认{config['serial']['port']}）：").strip() or config["serial"]["port"]
    ...
    try:
        ...
        while True:
            ...
            tts_total = 0.0
            for sentence_index, sentence in enumerate(sentence_chunks(streamer), 1):
                if first_text_seconds is None:
                    first_text_seconds = time.perf_counter() - llm_started
                response_parts.append(sentence)
                wav = tts_dir / f"turn_{turn:02d}_{sentence_index:02d}.wav"
                if config["tts"].get("output") == "board_speaker":
                    tts = tts_stream_to_board(
                        queue, f"{turn:02d}_{sentence_index:02d}", sentence, wav, ser,
                        config["tts"].get("stream_chunk_bytes", 4096),
                    )
                else:
                    tts = tts_request(queue, f"{turn:02d}_{sentence_index:02d}", sentence, wav)
